#===============================================================================
# Author: [Rana Bamdadi]
# Description: This script estimates the parameters of a New-Geometric time series 
#              model using the Whittle likelihood function and evaluates model 
#              performance with various error metrics.
#===============================================================================
############################################
################## NG-MINBL(1,0,1,1) model 
################## New-Geometric Modified Integer-Valued Bilinear
############################################
# Clear the workspace
rm(list = ls(all = TRUE))
...
fn <- function(a) 
  c(
    F1=a[1]*a[2]*a[3]*(a[3]+1)+a[3]-mean(x)*(1-g2/g1),
    F2=(a[1]+a[1]*a[2]*a[3])-g2/g1,
    F3=a[1]*a[2]*(2*(a[3]^3)+3*(a[3]^2)+a[3]+a[3]*(a[3]+1)*mean(x))-(g1-(g2/g1)*g0)
  )
ss <- multiroot(f = fn, start =c(0.364345,-0.434774,0.661315))
